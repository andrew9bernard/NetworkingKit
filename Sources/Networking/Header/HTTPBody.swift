...
public enum HTTPBody {
    ...
    case graphQL(_queryPayload: QueryPayload)
    ...
    var data: Data? {
        ...
        case .graphQL(let queryPayload):
            do {
                let payload = try JSONEncoder().encode(queryPayload)
                return payload
            } catch {
                return nil
            }
...
extension HTTPBody: Equatable {
    public static func ==(lhs: HTTPBody, rhs: HTTPBody) -> Bool {
        ...
        case (.graphQL(let payload1), .encodable(let payload2)):
            return String(describing: payload1) == String(describing: payload2)
